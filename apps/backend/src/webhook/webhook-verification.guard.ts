...
export const WebhookProvider = (provider: string) => {
  return (target: any, key?: string, descriptor?: any) => {
    if (descriptor) {
      // Method decorator
      Reflect.defineMetadata(WEBHOOK_PROVIDER_KEY, provider, descriptor.value);
    } else {
      // Class decorator
...
export class WebhookVerificationGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    if (!providerName) {
      providerName =
        request.query?.provider || request.headers['x-webhook-provider'];
    }
    ...
    const timestampHeader = request.headers[timestampHeaderName];
